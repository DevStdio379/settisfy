...
const MyBookings = ({ navigation }: MyBookingsScreenProps) => {
    ...
    const fetchData = async () => {
        if (user?.uid) {
            ...
            const activeBookings = myBookingsData.filter(listing => listing.status !== 6 && Number(listing.status) < 10);
            const inactiveBookings = myBookingsData.filter(listing => listing.status === 6 || Number(listing.status) >= 10);
                                                        ...
                                                        <TouchableOpacity
                                                            ...
                                                            <View style={[GlobalStyleSheet.flexcenter, { justifyContent: 'flex-start' }]}>
                                                                ...
                                                                <View style={{ width: '70%', padding: 10 }}>
                                                                    ...
                                                                    <Text style={{ fontSize: 14, color: COLORS.black, opacity: .5 }}>{data.settlerId ? `provided by ${data.settlerFirstName} ${data.settlerLastName}` : 'No service provider assigned'}</Text>
                                                        ...
                                                        <TouchableOpacity
                                                            ...
                                                            <View style={[GlobalStyleSheet.flexcenter, { justifyContent: 'flex-start' }]}>
                                                                ...
                                                                <View style={{ width: '70%', padding: 10 }}>
                                                                    <Text style={{ fontSize: 12, color: COLORS.black, opacity: .5 }}>{data.id}</Text>
                                                                    <Text numberOfLines={1} style={{ fontSize: 16, color: COLORS.black, fontWeight: 'bold' }}>{data.catalogueService.title}</Text>
                                                                    <Text style={{ fontSize: 14, color: COLORS.black, opacity: .5 }}>provided by {data.settlerFirstName} {data.settlerLastName}</Text>
                                                                    <Text style={{ fontSize: 14 }}>{new Date(data.selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' })}, {new Date(data.selectedDate).toLocaleDateString('en-GB', { weekday: 'long' })}</Text>
                                                                </View>
                                                            </View>
                                                        </TouchableOpacity>
